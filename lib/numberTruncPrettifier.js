...
/**
  ...
  * multiply the number per 10, truncate it and divide per 10. If the exercise was asking one decimal in all cases we
  * could use toFixed.
*/
function getAbbrValue(value, group) {
  return Math.trunc(value / Math.pow(1000,group) * 10) / 10;
}
...
function numberPrettifier(value) {
  ...
  if (size > 6 && size < 16) {
    ...
    let absValue = Math.trunc(Math.abs(value));
